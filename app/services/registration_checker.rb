...
class RegistrationChecker
  ...
  class << self
    ...
    def is_admin_or_current_user?
      @registration.present? ? request = @update_request : request = @registration_request
      # Only an admin or the user themselves can create a registration for the user
    ...
    def user_is_admin?
      UserApi.can_administer?(@requester_user_id, @competition_id)
    end
    ...
    def validate_update_status!
      return true unless @update_request.key?(:competing) && @update_request[:competing].key?(:status)

      status = @update_request[:competing][:status]

      puts "status: #{status}"
      puts user_is_admin?
      raise RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) unless status == 'cancelled' || user_is_admin?

      if status == 'cancelled' && (@registration.competing_status == 'accepted' && !@competition_info.user_can_cancel? && !user_is_admin?)
        raise RegistrationError.new(:unauthorized, ErrorCodes::ADMIN_MUST_CANCEL_REGISTRATION)
      end
      true
    end
  end
