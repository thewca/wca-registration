...
export default function RegistrationRequirements({ nextStep }) {
  ...
  return (
    <Segment padded inverted color="orange" attached size="big">
      <Accordion inverted>
        ...
        <Accordion.Content active={activeIndex === 1}>
          {competitionInfo.competitor_limit ? (
            ...
            <p>
              There is no competitor limit, but if this competition has multiple
              locations then each location may have a separate competitor limit.
              Please review the other tabs for more information.
            </p>
          )}
        </Accordion.Content>
        ...
        >
          ...
          {competitionInfo.refund_policy_percent === 0
            ? 'No refunds'
            : `${competitionInfo.refund_policy_percent}% Refund before ${moment(
      ...
      <Button positive onClick={() => nextStep()}>
        I agree!
      </Button>
    </Segment>
  )
